...
const Story = () => {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      sparkles = Array.from({ length: window.innerWidth < 640 ? 20 : 40 }, () => new Sparkle(r.width, r.height));
    };
  ...
  return (
    <section
      ref={ref}
      id="story"
      className="relative w-full overflow-hidden bg-gradient-to-b from-[#f8f4ed]/80 via-[#fdfcfb]/70 to-[#f8f4ed]/80 py-16 sm:py-20 md:py-28 px-4 sm:px-6 md:px-12 lg:px-24"
    >
      <canvas ref={canvasRef} className="absolute inset-0 -z-10 w-full h-full opacity-60 md:opacity-80" />

      <motion.div
        className="max-w-6xl mx-auto space-y-14 md:space-y-24"
        variants={container}
        initial="hidden"
        animate={controls}
      >
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          ...
          <div className="space-y-6">
            {[
              { title: "First Glance", text: "We met at Ganesh Chaturthi celebrations in 2022. Amid modak and aarti, our eyes met across the mandap." },
              { title: "The Proposal", text: "Under the full moon at Shaniwar Wada, Arjun proposed with his grandmother’s antique ring." },
            ].map((s, i) => (
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6 max-w-md mx-auto">
            {[photo1, photo2].map((img, i) => (
              <motion.div
                key={i}
                variants={card}
                className={`rounded-2xl overflow-hidden border-4 border-[#daa520]/50 shadow-xl ${i === 1 ? "mt-8 sm:mt-12" : ""}`}
              >
